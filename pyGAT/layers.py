...
class DP_layer(torch.nn.Module):
    ...
    def forward(self, h):
        ...
        noise = lap_noise(h, self.lbd) * divisor
        return h + noise
...
class perturb_adj(torch.nn.Module):
    ...
    def forward(self, adj):
        Ne = adj.sum() // 2
        Nu = adj.shape[0] * (adj.shape[0] - 1) // 2
        p1 = Nu/(np.exp(self.gamma) * Ne + Nu)
        p2 = Ne/(np.exp(self.gamma) * Ne + Nu)
