...
def load_data(datapath_str, dataset_str, ptb=False):
    ...
    elif "graph" in dataset_str:
        return load_sub_graph_data(datapath_str, dataset_str)
    else:
        raise Exception("Invalid dataset!", dataset_str)

def load_sub_graph_data(datapath_str, dataset_str):
    pkl_file = f"{datapath_str}graph_detail.pkl"
    with open(pkl_file, 'rb') as f:
        if sys.version_info > (3, 0):
            dict = pkl.load(f, encoding='latin1')
        else:
            dict = pkl.load(f)
    return dict['adj'], dict['ft'], None, None, None, None, None, None
...
def load_data_original(datapath_str, dataset_str):
    ...
    for i in range(len(names)):
        with open("dataset/{}/ind.{}.{}".format(dataset_str, dataset_str, names[i]),
                    'rb') as f:
            if sys.version_info > (3, 0):
    ...
    test_idx_reorder = parse_index_file("dataset/{}/ind.{}.test.index".format(
...
def load_data_social(datapath_str, dataset_str, ptb=False):
    ...
    ind.dataset_str.allx => the feature vectors of both labeled and unlabeled training instances
    ind.dataset_str.ally => the labels for instances in ind.dataset_str.allx as numpy.ndarray object;
    dataset_str.adj => adjecancy matrix.
    All objects above must be saved using python pickle module.
...
def load_data_Fb(datapath_str, dataset_str):
    """
    Loads input data from dataset directory
