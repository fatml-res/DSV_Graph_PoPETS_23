...
if __name__ == "__main__":
    ...
    for gamma in [10.0, 5.0, 3.0, 1.0, 0.5]:
        for t in range(5):
            ...
            while tmp_list[-4] < 0.2 and count < 5:
                count += 1
                print("Try {} time for gamma={}".format(count, gamma))
                tmp_list = run_GAT(gender, ft, adj, labels, epochs=300, dataset=dataset,
                                   DP=False, lbd=0,
                                   Ptb=True, gamma=gamma,
                                   saving_path="GAT/ptb_adj/gamma={}/t={}".format(gamma, t))
            if tmp_list[-4] > 0.2:
                acc_lists.append(tmp_list)
            else:
                print("Target model with DP failed for gamma={} and t={} for {} times".format(gamma, t, count))
    ...
    df_acc_avg.to_csv("GAT/ptb_adj/{}_target_acc_agg.csv".format(dataset))
    pass